...
**When you need this.** Use this topic when you're planning the full data-driven design flow in Illustrator: template document, data file, variables, previews, and optional desktop batch export. Understanding merge on the desktop helps you design templates that behave predictably when merged at scale through the API.

Adobe HelpX explains how to merge data efficiently to create dynamic, personalized designs:

* Create an Illustrator document to use as the template.
* Prepare a source file in CSV or XML format.
* Saving a **data merge template** when you define variables, including saving to **SVG** with **Include Adobe Graphics Server Data** for variable substitution in downstream workflows.
...
Data source files should be saved as comma-delimited (`.csv`) or XML. For CSV, records are separated by paragraph breaks and fields by commas or tabs; the file can include text or paths that point to images.

For XML, HelpX covers defining variables, using the **Data Set** control in the **Variables** panel, saving and loading variable libraries, and editing the XML in a text editor.
...
You can import a data source in the **Variables** panel and bind variables to your data. Only one data source file can be selected per document.

HelpX also includes tips for navigating data sets and a note about UTF-8 CSV files from Excel when double-byte characters are involved.
...
You can edit a variable’s name or type, unbind a variable from its object, or lock variables to restrict certain edits while still allowing binding changes.
...
The HelpX topic explains:

* Setting **XML ID** in preferences.
* How to select bound objects from the **Variables** panel menu.
* Edit text on the artboard, replace linked images, edit graph data, or change visibility in the **Layers** panel for visibility variables.
